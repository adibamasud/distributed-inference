...
MODELS_CONFIG = {
    "AlexNet": {
        ...
        # AlexNet features have 13 layers. Splitting around layer 11 gives roughly 80%.
        "part1_definition": lambda model: nn.Sequential(*list(model.features.children())[:11]), # Changed from :10 to :11 for ~80%
    ...
    "InceptionV3": {
        ...
        "part1_definition": lambda model: nn.Sequential(
            ...
            model.Mixed_6a,
            model.Mixed_6b,
            model.Mixed_6c, # Added for ~80%
            model.Mixed_6d  # Added for ~80%
        ),
    ...
    "MobileNetV2": {
        ...
        # MobileNetV2 features have 19 layers. Splitting around layer 15 gives roughly 80%.
        "part1_definition": lambda model: nn.Sequential(*list(model.features.children())[:15]), # Changed from :14 to :15 for ~80%
        "transform_resize": (224, 224),
    ...
    "ResNet18": {
        ...
        "part1_definition": lambda model: nn.Sequential(
            ...
            model.layer1, model.layer2, model.layer3, # Retained from 70/30
            # For ResNet18, layer4 is a significant portion. Putting it entirely on client is ~25%.
            # To get closer to 80/20, we'll keep layer3 on server.
            # Splitting a ResNet block further requires more intricate changes,
            # so keeping layer3 on server and layer4 on client is a practical 80/20 equivalent.
            # If a finer split of layer4 is needed, it would involve modifying layer4 itself.
        ),
        "transform_resize": (224, 224),
    ...
    "VGG16": {
        ...
        # VGG16 features have 31 layers. Splitting around layer 25 gives roughly 80%.
        "part1_definition": lambda model: nn.Sequential(*list(model.features.children())[:25]), # Changed from :22 to :25 for ~80%
        "transform_resize": (224, 224),
