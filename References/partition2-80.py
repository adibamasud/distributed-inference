...
MODELS_CONFIG = {
    "AlexNet": {
        ...
        # AlexNet features have 13 layers. Starts from where server left off (:11)
        "part2_definition": lambda model: nn.Sequential(
            *list(model.features.children())[11:], # Continue from server's new split point
            model.avgpool,
            nn.Flatten(),
            model.classifier
        ),
    ...
    "InceptionV3": {
        ...
        "part2_definition": lambda model: nn.Sequential(
            model.Mixed_6e, # Starts from where server left off (after Mixed_6d)
            model.Mixed_7a,
            model.Mixed_7b,
            model.Mixed_7c,
            model.avgpool,
    ...
    "MobileNetV2": {
        ...
        # MobileNetV2 features have 19 layers. Starts from where server left off (:15)
        "part2_definition": lambda model: nn.Sequential(
            *list(model.features.children())[15:], # Continue from server's new split point
    ...
    "ResNet18": {
        ...
        "part2_definition": lambda model: nn.Sequential(
            model.layer4, # Remains as the client's part for ~20%
            model.avgpool,
            nn.Flatten(),
            model.fc
        ),
    ...
    "VGG16": {
        ...
        # VGG16 features have 31 layers. Starts from where server left off (:25)
        "part2_definition": lambda model: nn.Sequential(
            *list(model.features.children())[25:], # Continue from server's new split point
            model.avgpool,
            nn.Flatten(),
            model.classifier
        ),
